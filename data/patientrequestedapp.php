...
$sql = "select * 
		from reqapp
        where (pid='$pid' or did='$pid') and status=0  
        order by rdate; ";  
...
while($i<$Nrow){
    ...
    <td>
        <form action='deleterapp' method=POST><input type='text' name='did' hidden value='$rid'> <input type='submit' style='width:100%;background-color:red' value='Delete'></form>
    </td>
    </tr>";
    $i=$i+1;
